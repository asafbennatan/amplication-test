...
import { Prisma, Message, Chat, User } from "@prisma/client";
...
export class MessageServiceBase {
  ...
  async getFromUser(parentId: string): Promise<User | null> {
    return this.prisma.message
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getToUser(parentId: string): Promise<User | null> {
    return this.prisma.message
      .findUnique({
        where: { id: parentId },
      })
